...
def tokenize_sentence(sentence):#split into sentences
    return ' '.join(list(tokenize(sentence)))

def article_is_complete(article):# check for heading and description
...
def filter(line, whitelist): # filter out blacklisted words
    return ''.join([ch for ch in line if ch in whitelist])

def filter_length(headings, descriptions):# make sure words are within a certain length
...
def index_data(tokenized_sentences, vocab_size):# Forms vocab, and idx2word and word2idx dicts   
...
def pad_seq(seq, lookup, max_length): #pad sequence and labels with ceros
    indices = []
